...
bot.start(async (ctx) => {
  try {
    ...
    await ctx.reply(
      ...
      `CitizensVpnBot - бот выдачи файлов конфигурации OpenVPN по коду.\n\n`, +

      `⚠️ Предупреждение\n\n` +
      `CITIZENSVPN предоставляет файлы конфигурации исключительно в технических целях.\n\n` +
      `Сервис не несёт ответственности за использование предоставленных файлов конфигурации,` +
      `а также за любые последствия, возникшие в результате их применения.\n` +
      `Пользователь самостоятельно несёт полную ответственность за соблюдение законодательства страны,\n` +
      `на территории которой используются файлы конфигурации и сервис в целом.\n` +
      `Использование файлов конфигурации должно осуществляться строго\n` +
      `в соответствии с действующими правовыми нормами государства пребывания.\n\n` +

      `Перед использованием сервиса рекомендуем ознакомиться с законодательными актами,\n` +
      `регулирующими применение технологий туннелирования и шифрования трафика\n` +
      `в вашей юрисдикции.\n`,
